...
local originalPos=ar.UIElements.Main.Position
task.spawn(function()
local intensity=6
local duration=0.3
...
ar.UIlements.Main.Position=originalPos+UDim2.new(0,x,0,y)
task.wait()
end
...
if currentContainer and not currentContainer:FindFirstChild("SnowLayer") then
local snowFunc=a.load'd'.CreateSnowLayer
if snowFunc then snowFunc(currentContainer,true) end
end
ao.OnChangeFunc(aq)
end
end
...
do
local openButtonFrame=aw.OpenButtonMain.Button
if openButtonFrame then
local snowLayer=a.load'd'.CreateSnowLayer(openButtonFrame,true)
...
task.wait(0.06)
aw.Closed=false
if not aw.UIElements.Main:FindFirstChild("WindowSnowLayer") then
local snowFx=a.load'd'.CreateSnowLayer(aw.UIElements.Main,false)
