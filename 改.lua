...
function r.CreateSnowLayer(parentFrame, isEdgeOnly)
    ...
    snowContainer.Parent = parentFrame

    local snowflakes = {}
    local flakeCount = isEdgeOnly and 35 or 60
    ...
    if not ar.Locked then
        ...
        task.spawn(function()
            ...
            while tick() - startTime < duration do
                ...
                local x = math.sin(elapsed * 60) * 8 * (1 - elapsed/duration)
                local y = math.cos(elapsed * 50) * 8 * (1 - elapsed/duration)
                ar.UIElements.Main.Position = originalPos + UDim2.new(0, x, 0, y)
                task.wait()
            end
            ar.UIElements.Main.Position = originalPos
        end)
        ao:SelectTab(as)
    end
end)
...
if currentContainer and not currentContainer:FindFirstChild("SnowLayer") then
    ...
    if snowFunc then snowFunc(currentContainer, false) end
end
...
task.defer(function()
    if aw.OpenButtonMain and aw.OpenButtonMain.Button then
        a.load'd'.CreateSnowLayer(aw.OpenButtonMain.Button, true)
    end
end)
...
if snowFx then snowFx.Visible = true end
...
local snowFx = aw.UIElements.Main:FindFirstChild("WindowSnowLayer")
if snowFx then snowFx.Visible = false end
end
