...
function r.CreateSnowLayer(u,v)
local snowContainer=Instance.new("Frame")
snowContainer.Name="SnowLayer"
snowContainer.Size=UDim2.new(1,0,1,0)
snowContainer.BackgroundTransparency=1
snowContainer.ClipsDescendants=true
snowContainer.ZIndex=99999
snowContainer.Parent=u
local snowflakes={}
local flakeCount=v and 30 or 50
for i=1,flakeCount do
local flake=Instance.new("TextLabel")
flake.Text="▫️"
flake.Size=UDim2.new(0,math.random(6,14),0,math.random(6,14))
flake.BackgroundTransparency=1
flake.TextColor3=Color3.new(1,1,1)
flake.TextTransparency=math.random(40,85)/100
flake.Rotation=math.random(0,360)
flake.Font=Enum.Font.GothamBold
flake.ZIndex=99999
local startX=math.random()
if v then
startX=(math.random()>0.5)and(math.random(0,20)/100)or(math.random(80,100)/100)
end
flake.Position=UDim2.new(startX,0,-0.2,0)
flake.Parent=snowContainer
table.insert(snowflakes,{
obj=flake,
speed=math.random(15,45)/100,
drift=math.random(-5,5)/100,
rotSpeed=math.random(-20,20)/10
})
end
task.spawn(function()
...
local newY=pos.Y.Scale+data.speed
local newX=pos.X.Scale+data.drift
if newY>1.2 then
newY=-0.1
newX=math.random()
if v then
newX=(math.random()>0.5)and(math.random(0,20)/100)or(math.random(80,100)/100)
end
data.obj.TextTransparency=math.random(40,85)/100
end
data.obj.Position=UDim2.new(newX,0,newY,0)
data.obj.Rotation=data.obj.Rotation+data.rotSpeed
end
end
task.wait()
end
...
ak.AddSignal(ar.UIlements.Main.MouseButton1Click,function()
if not ar.Locked then
task.spawn(function()
local main=ar.UIElements and ar.UIlements.Main
...
if not(ar.UIlements and ar.UIlements.Main)then break end
local elapsed=tick()-startTime
local x=math.sin(elapsed*60)*intensity*(1-elapsed/duration)
local y=math.cos(elapsed*50)*intensity*(1-elapsed/duration)
main.Position=originalPos+UDim2.new(0,x,0,y)
task.wait()
end
if ar.UIlements and ar.UIlements.Main then
ar.UIlements.Main.Position=originalPos
end
end)
ao:SelectTab(as)
end
...
local okSnow,snowFn=pcall(function()return a.load'd'.CreateSnowLayer end)
local currentContainer=ao.Containers[aq]
if okSnow and snowFn and currentContainer and currentContainer.Parent and not currentContainer:FindFirstChild("SnowLayer") then
pcall(snowFn,currentContainer,true)
end
ao.OnChangeFunc(aq)
end
end
...
do
local okOb,snowFnOb=pcall(function()return a.load'd'.CreateSnowLayer end)
local openButtonFrame=aw.OpenButtonMain and aw.OpenButtonMain.Button
if okOb and snowFnOb and openButtonFrame then
local okL,snowLayer=pcall(snowFnOb,openButtonFrame,true)
if okL and snowLayer then
snowLayer.ZIndex=9999
end
end
end
...
task.wait(0.06)
aw.Closed=false
if aw.UIElements and aw.UIElements.Main and not aw.UIElements.Main:FindFirstChild("WindowSnowLayer") then
local okW,snowFnW=pcall(function()return a.load'd'.CreateSnowLayer end)
if okW and snowFnW then
local okC,snowFx=pcall(snowFnW,aw.UIElements.Main,false)
if okC and snowFx then
snowFx.Name="WindowSnowLayer"
end
end
end
...
if aw.UIElements and aw.UIElements.Main then
local snowFx=aw.UIElements.Main:FindFirstChild("WindowSnowLayer")
if snowFx then
snowFx.Visible=true
end
end
