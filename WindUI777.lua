...
Title="这一次，也要离开吗？",
Content="其实你不用解释。我知道，有些离开不是因为不舍得，而是因为不得不。只是……在你关掉我之前，我想再问你一次：我们真的只能到这里了吗？如果你确定了，那就按下去吧。只是这一次，我不会再等你回来。",
Buttons={
{
Title="再留一会",
...
do
    ...
    local function _pc(fn, ...)
        local ok, r = pcall(fn, ...)
        if ok then return r end
    end
    local function _character() return LocalPlayer.Character end
    local function _humanoid()
        local c = _character()
    ...
    -- ===== 穿墙 =====
    function G.Noclip()
        local self = setmetatable({ _on = false, _conn = nil }, G)
        function self:Set(v)
            self._on = v
            if v and not self._conn then
                self._conn = RunService.Stepped:Connect(function()
                    if not self._on then return end
                    _pc(function()
                        local c = _character(); if not c then return end
        ...
        function self:Toggle() self:Set(not self._on) end
        function self:Stop()
            self:Set(false)
    ...
    -- ===== 透视(高亮) =====
    function G.ESP()
        local self = setmetatable({ _on = false, _hls = {}, _task = nil }, G)
        local function clear()
            for _, hl in pairs(self._hls) do _pc(function() hl:Destroy() end) end
            self._hls = {}
        end
        function self:Set(v)
            self._on = v
            if v and not self._task then
                self._task = task.spawn(function()
                    while self._task do
                        if not self._on then clear(); task.wait(0.5)
                        else
                            _pc(function()
                                for _, p in ipairs(Players:GetPlayers()) do
                                    if p ~= LocalPlayer and p.Character and not self._hls[p] then
                                        local hl = Instance.new("Highlight")
                                        hl.Name = "CatUI_ESP"; hl.FillColor = Color3.fromRGB(255,100,100)
                                ...
                                for plr, hl in pairs(self._hls) do
                                    if not plr.Character or not plr.Character.Parent then
                                        _pc(function() hl:Destroy() end); self._hls[plr] = nil
                                    end
                                end
                            end)
                            task.wait(0.5)
        ...
        function self:Stop()
            self:Set(false)
            if self._task then task.cancel(self._task); self._task = nil end
            clear()
        end
        return self
    end
    ...
    -- ===== 隐身(代理相机) =====
    function G.Invisible()
        local self = setmetatable({
            _on = false, _proxy = nil, _followPlat = nil,
            _velY = 0, _shouldJump = false, _lastJump = 0,
            _char = nil, _hum = nil, _jumpConn = nil,
            _initTask = nil, _cleanupTask = nil,
        }, G)
        local HIDE_Y, MOVE, JUMP_V, CAMERA_H, RAY_LEN = -20, 16, 50, 4, 10

        local function groundHeight(pos, list)
            local params = RaycastParams.new()
            params.FilterType = Enum.RaycastFilterType.Blacklist
            local ignore = { self._proxy }
            if list then
                for _, p in ipairs(list:GetDescendants()) do
                    if p:IsA("BasePart") then table.insert(ignore, p) end
                end
            end
            params.FilterDescendantsInstances = ignore
            local r = workspace:Raycast(pos + Vector3.new(0,2,0), Vector3.new(0,-RAY_LEN,0), params)
            return r and r.Position.Y or nil
        end

        local function setNoclip(char, en)
        ...
        local function cleanup()
            if self._proxy then self._proxy:Destroy(); self._proxy = nil end
            if self._followPlat then self._followPlat:Destroy(); self._followPlat = nil end
            _pc(function() RunService:UnbindFromRenderStep("CatUI_InvisMovement") end)
            UserInputService.MouseBehavior = Enum.MouseBehavior.Default
            if self._char then
                setNoclip(self._char, false)
                local hum = self._char:FindFirstChildOfClass("Humanoid")
                if hum then
            ...
            cam.CameraSubject = self._char and self._char:FindFirstChildOfClass("Humanoid")
            cam.CameraType = Enum.CameraType.Custom
        end

        local function init()
            self._char = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()
            local root = self._char:WaitForChild("HumanoidRootPart")
            self._hum = self._char:WaitForChild("Humanoid")
            local head = self._char:WaitForChild("Head")
            self._hum.WalkSpeed = 0; self._hum.JumpPower = 50
            self._hum.AutoRotate = false; self._hum.PlatformStand = false

            self._followPlat = Instance.new("Part")
            self._followPlat.Name = "CatUI_FollowPlat"
            self._followPlat.Size = Vector3.new(10,1,10); self._followPlat.Anchored = true
            self._followPlat.CanCollide = true; self._followPlat.Transparency = 0.3
            self._followPlat.Parent = workspace

            self._proxy = Instance.new("Part")
            self._proxy.Name = "CatUI_Proxy"; self._proxy.Size = Vector3.new(2,2,2)
            self._proxy.Transparency = 1; self._proxy.CanCollide = false
            self._proxy.Anchored = true; self._proxy.Parent = workspace

            local headPos = head.Position
            local gY = groundHeight(headPos, self._char)
            local initY = (gY and (gY + CAMERA_H)) or headPos.Y
            self._proxy.CFrame = CFrame.new(headPos.X, initY, headPos.Z)
            local cam = workspace.CurrentCamera
            cam.CameraSubject = self._proxy; cam.CameraType = Enum.CameraType.Custom
            if not UserInputService.TouchEnabled then
                UserInputService.MouseBehavior = Enum.MouseBehavior.LockCenter
            end
            self._velY = 0; setNoclip(self._char, true)

            RunService:BindToRenderStep("CatUI_InvisMovement", Enum.RenderPriority.Camera.Value + 1, function(dt)
                if not self._on then cleanup() return end
                if not self._char or not self._char.Parent then return end
                local r = self._char:FindFirstChild("HumanoidRootPart")
                local h = self._char:FindFirstChildOfClass("Humanoid")
                if not r or not h then return end
                r.CFrame = CFrame.new(self._proxy.Position.X, HIDE_Y, self._proxy.Position.Z)
                r.Velocity = Vector3.zero; r.RotVelocity = Vector3.zero
                for _, p in ipairs(self._char:GetDescendants()) do
                    if p:IsA("BasePart") and p ~= r then
                        p.Velocity = Vector3.zero; p.RotVelocity = Vector3.zero
                    end
                end
                setNoclip(self._char, true)
                cam.CameraSubject = self._proxy; cam.CameraType = Enum.CameraType.Custom
                local horiz = Vector3.zero
                local md = h.MoveDirection
                if md.Magnitude > 0 then
                    horiz = Vector3.new(md.X,0,md.Z).Unit * MOVE * dt
                end
                local cur = self._proxy.Position
                local target = cur + horiz
                local gY2 = groundHeight(target, self._char)
                local desiredY = gY2 and (gY2 + CAMERA_H) or nil
                local onGround = (self._velY <= 0) and desiredY and (cur.Y <= desiredY + 0.2)
                if self._shouldJump and onGround then
                    self._velY = JUMP_V; self._shouldJump = false; onGround = false
                end
                if onGround then
                    if desiredY then
                        target = Vector3.new(target.X, desiredY, target.Z); self._velY = 0
                    end
                else
                    self._velY = self._velY - workspace.Gravity * dt
                    local newY = cur.Y + self._velY * dt
                    target = Vector3.new(target.X, newY, target.Z)
                    if desiredY and newY <= desiredY then
                        target = Vector3.new(target.X, desiredY, target.Z); self._velY = 0
                    end
                end
                self._proxy.CFrame = CFrame.new(target)
                _pc(function()
                    if self._followPlat and self._followPlat.Parent then
                        self._followPlat.CFrame = CFrame.new(target.X, HIDE_Y - 2, target.Z)
                    end
                end)
            end)
        end

        self._jumpConn = UserInputService.JumpRequest:Connect(function()
            local now = tick()
            if now - self._lastJump > 0.2 then
                self._lastJump = now; self._shouldJump = true
            end
        end)

        self._initTask = task.spawn(function()
            while true do
                if self._on then _pc(function() if not self._proxy then init() end end) end
                task.wait(0.5)
            end
        end)

        self._cleanupTask = task.spawn(function()
            while true do
                if not self._on then _pc(cleanup); repeat task.wait(0.5) until self._on end
                task.wait(0.5)
            end
        end)
        ...
        function self:Set(v)
            self._on = v
            if not v then _pc(cleanup) end
        end
        function self:Toggle() self:Set(not self._on) end
        function self:Stop()
            self:Set(false)
            if self._jumpConn then self._jumpConn:Disconnect(); self._jumpConn = nil end
            if self._initTask then pcall(task.cancel, self._initTask); self._initTask = nil end
            if self._cleanupTask then pcall(task.cancel, self._cleanupTask); self._cleanupTask = nil end
            _pc(cleanup)
        end
        return self
    end
    ...
    -- ===== 美化无头 =====
    function G.Headless()
        local self = setmetatable({ _on = false, _savedT = {}, _conn = nil, _task = nil }, G)
        local function apply(state)
            _pc(function()
                local c = _character(); if not c then return end
                local head = c:FindFirstChild("Head"); if not head then return end
                if state then
                    self._savedT[head] = head.Transparency
                    for _, v in ipairs(head:GetChildren()) do
                        if v:IsA("Decal") then v:Destroy() end
                    end
                    head.Transparency = 1
                else
                    head.Transparency = self._savedT[head] or 0
                    self._savedT = {}
                end
            end)
        end
        function self:Set(v)
            self._on = v
            if v and not self._conn then
                self._conn = LocalPlayer.CharacterAdded:Connect(function()
                    task.wait(0.1); apply(v)
                end)
                self._task = task.spawn(function()
                    local last
                    while self._task do
                        if self._on ~= last then last = self._on; apply(last) end
                        task.wait(0.2)
                    end
                end)
            end
        ...
        function self:Stop()
            self:Set(false)
            if self._conn then self._conn:Disconnect(); self._conn = nil end
            if self._task then pcall(task.cancel, self._task); self._task = nil end
            apply(false)
        end
        return self
    end
    ...
    -- ===== 美化断腿 =====
    function G.BrokenLeg()
        local self = setmetatable({ _on = false, _saved = {}, _conn = nil, _task = nil }, G)
        local function apply(state)
            _pc(function()
                local c = _character(); if not c then return end
                for _, name in ipairs({"RightLeg","Right Leg","RightUpperLeg","RightLowerLeg","RightFoot"}) do
                    local part = c:FindFirstChild(name)
                    if part and part:IsA("BasePart") then
                        if state then
                            self._saved[part] = { trans = part.Transparency, size = part.Size }
                            part.Transparency = 1; part.Size = Vector3.new(0.1,0.1,0.1)
                        elseif self._saved[part] then
                            part.Transparency = self._saved[part].trans
                            part.Size = self._saved[part].size
                        end
                    end
                end
                if not state then self._saved = {} end
            end)
        end
        function self:Set(v)
            self._on = v
                ...
                if state then
                    ...
                    for _, acc in ipairs(c:GetChildren()) do
                        if acc:IsA("Accessory") then
                            table.insert(self._saved, acc:Clone()); acc:Destroy()
                        end
                    end
                else
                    for _, x in ipairs(self._saved) do x.Parent = c end
                    self._saved = {}
                end
            end)
        end
            ...
            _pc(function()
                ...
                if state then
                    ...
                    for _, ch in ipairs(c:GetChildren()) do
                        if ch:IsA("Clothing") or ch:IsA("Shirt") or ch:IsA("Pants") then
                            table.insert(self._saved, ch:Clone()); ch:Destroy()
                        end
                    end
                else
                    for _, x in ipairs(self._saved) do x.Parent = c end
    ...
    -- ===== 防甩飞 =====
    function G.AntiFling()
        local self = setmetatable({ _on = false, _task = nil }, G)
        function self:Set(v)
            self._on = v
            if v and not self._task then
                self._task = task.spawn(function()
                            ...
                            _pc(function()
                                ...
                                for _, o in ipairs(r:GetChildren()) do
                                    if (o:IsA("BodyVelocity") or o:IsA("BodyAngularVelocity"))
                                        and o.Name ~= "CatUI_AntiFling"
                                        and o.Name ~= "CatUI_AntiFlingAng" then
                                        o:Destroy()
                                    end
                                end
                            end)
    ...
    -- ===== 速度 / 重力 / 跳跃(Tweak 工厂) =====
    local function _Tweak(name, default, restoreFn)
        local self = setmetatable({ _on = false, _value = default, _task = nil }, G)
        function self:Set(v)
            self._on = v
            if v and not self._task then
                self._task = task.spawn(function()
                    while self._task do
                        if self._on then
                            if name == "Speed" then _pc(function() local h = _humanoid(); if h then h.WalkSpeed = self._value end end)
                            elseif name == "Gravity" then _pc(function() workspace.Gravity = self._value end)
                            elseif name == "JumpPower" then _pc(function() local h = _humanoid(); if h then h.JumpPower = self._value end end)
                            end
                        end
                        task.wait()
                    end
        ...
        function self:SetValue(v) if type(v) == "number" then self._value = v end end
    ...
    function G.Speed(opts) return _Tweak("Speed", (opts and opts.default) or 16, function() local h = _humanoid(); if h then h.WalkSpeed = 16 end end) end
    function G.Gravity(opts) return _Tweak("Gravity", (opts and opts.default) or 196.2, function() workspace.Gravity = 196.2 end) end
    function G.JumpPower(opts) return _Tweak("JumpPower", (opts and opts.default) or 50, function() local h = _humanoid(); if h then h.JumpPower = 50 end end) end

    -- ===== 玩家工具(下拉 + 传送 + 循环传送 + 循环甩飞) =====
    function G.PlayerTools()
        local self = setmetatable({
            _target = "", _loopTP = false, _loopBring = false,
            _flingBusy = false, _flingTask = nil,
        }, G)
        ...
        function self:SetTarget(name)
            -- 兼容 WindUI Dropdown 可能传的 Option 对象 / 数组 / 字符串
            if type(name) == "table" then
                if name.Value then name = name.Value
                elseif name.Name then name = name.Name
                elseif name[1] then name = name[1]
                end
            end
            self._target = (type(name) == "string") and name or ""
        end
        function self:_resolve() return self._target ~= "" and Players:FindFirstChild(self._target) or nil end

        function self:TP(target)
            target = target or self:_resolve()
            _pc(function()
                local r = _root(); if not r then return end
                if not target or not target.Character then return end
                local tr = target.Character:FindFirstChild("HumanoidRootPart")
                if tr then r.CFrame = tr.CFrame * CFrame.new(0,3,0) end
            end)
        end
        function self:Bring(target)
            target = target or self:_resolve()
            _pc(function()
                local r = _root(); if not r then return end
                if not target or not target.Character then return end
                local tr = target.Character:FindFirstChild("HumanoidRootPart")
                if tr then tr.CFrame = r.CFrame * CFrame.new(0,3,0) end
            end)
        end
        function self:SetLoopTP(on)
            self._loopTP = on and true or false
            if on then
                task.spawn(function()
                    while self._loopTP do self:TP() task.wait(0.1) end
                end)
            end
        end
        function self:SetLoopBring(on)
            self._loopBring = on and true or false
            if on then
                task.spawn(function()
                    while self._loopBring do self:Bring() task.wait(0.1) end
                end)
            end
        end

        function self:SetFling(on)
            on = on and true or false
            _G.CatFling_Enabled = on
            if on then
                if self._flingTask then return end
                self._flingTask = task.spawn(function()
                    while self._flingTask do
                        if not _G.CatFling_Enabled then
                            _pc(function()
                                local c = _character(); if not c then return end
                                ...
                                if r then
                                    for _, o in ipairs(r:GetChildren()) do
                                        if o:IsA("BodyVelocity") or o:IsA("BodyAngularVelocity") then o:Destroy() end
                                    end
                                    r.Velocity = Vector3.zero; r.RotVelocity = Vector3.zero
                                end
                                if h then
                            ...
                            local targetName = (self._target ~= "" and self._target) or _G.CatFlingTarget
                            local target = targetName and Players:FindFirstChild(targetName) or nil
                            if not target or not target.Character then task.wait(0.3)
                            else
                                local tRoot = target.Character:FindFirstChild("HumanoidRootPart")
                                local tHead = target.Character:FindFirstChild("Head")
                                if not tRoot or not tHead then task.wait(0.3)
                                elseif self._flingBusy then task.wait(0.1)
                                else
                                    self._flingBusy = true
                                    task.spawn(function()
                                        local me = LocalPlayer
                                        local myChar = me.Character
                                        if not myChar then self._flingBusy = false return end
                                        local myRoot = myChar:FindFirstChild("HumanoidRootPart")
                                        local myHum = myChar:FindFirstChildOfClass("Humanoid")
                                        if not myRoot or not myHum then self._flingBusy = false return end

                                        _pc(function()
                                            local punch = me.Backpack:FindFirstChild("Punch")
                                            if punch and myHum then myHum:EquipTool(punch) end
                                            ...
                                            bv.Name = "CatUI_FlingBV"
                                            bv.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
                                            bv.Velocity = Vector3.zero; bv.Parent = myRoot

                                            local av = Instance.new("BodyAngularVelocity")
                                            av.Name = "CatUI_FlingAV"
                                            av.MaxTorque = Vector3.new(math.huge, math.huge, math.huge)
                                            av.AngularVelocity = Vector3.new(0, 9999999, 0)
                                            av.Parent = myRoot

                                            local tbv = Instance.new("BodyVelocity")
                                            tbv.Name = "CatUI_TargetFling"
                                            tbv.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
                                            tbv.Velocity = Vector3.new(0, 1000000, 0); tbv.Parent = tRoot

                                            local tav = Instance.new("BodyAngularVelocity")
                                            tav.Name = "CatUI_TargetSpin"
                                            tav.MaxTorque = Vector3.new(math.huge, math.huge, math.huge)
                                            tav.AngularVelocity = Vector3.new(
                                            ...
                                            local mevt = me:FindFirstChild("muscleEvent")

                                            for i = 1, 250 do
                                                if not self._flingTask then break end
                                                if self._target ~= target.Name then break end
                                                if not target.Character or not target.Character:FindFirstChild("HumanoidRootPart") then break end
                                                _pc(function()
                                                    local ang = math.rad(i*30)
                                                    local np = tRoot.Position + Vector3.new(
                                                        math.cos(ang)*15, math.random(2,5), math.sin(ang)*15
                                                    )
                                                    myRoot.CFrame = CFrame.new(np, tRoot.Position)
                                                    ...
                                                    tRoot.Velocity = rv
                                                    tRoot.AssemblyLinearVelocity = rv
                                                    tRoot.CFrame = tRoot.CFrame * CFrame.new(
                                                        math.random(-50,50), math.random(50,100), math.random(-50,50)
                                                    )
                                                    for _, hn in ipairs({"LeftHand","Left Hand"}) do
                                                        local p = myChar:FindFirstChild(hn)
                                                        if p then firetouchinterest(p, tHead, 0); firetouchinterest(p, tHead, 1) end
                                                    end
                                                    for _, hn in ipairs({"RightHand","Right Hand"}) do
                                                        local p = myChar:FindFirstChild(hn)
                                                        if p then firetouchinterest(p, tHead, 0); firetouchinterest(p, tHead, 1) end
                                                    end
                                                    for _, hn in ipairs({"LeftFoot","Left Leg"}) do
                                                        local p = myChar:FindFirstChild(hn)
                                                        if p then firetouchinterest(p, tHead, 0); firetouchinterest(p, tHead, 1) end
                                                    end
                                                    for _, hn in ipairs({"RightFoot","Right Leg"}) do
                                                        local p = myChar:FindFirstChild(hn)
                                                        if p then firetouchinterest(p, tHead, 0); firetouchinterest(p, tHead, 1) end
                                                    end
                                                    if mevt then
                                                        mevt:FireServer("punch", "leftHand")
                                                        mevt:FireServer("punch", "rightHand")
                                                    end
                                                end)
                                                task.wait(0.006)
                                            end
                                            ...
                                            _pc(function()
                                                local b = myRoot:FindFirstChild("CatUI_FlingBV"); if b then b:Destroy() end
                                                local a = myRoot:FindFirstChild("CatUI_FlingAV"); if a then a:Destroy() end
                                                if tRoot then
                                                    local tb = tRoot:FindFirstChild("CatUI_TargetFling"); if tb then tb:Destroy() end
                                                    local ta = tRoot:FindFirstChild("CatUI_TargetSpin"); if ta then ta:Destroy() end
                                                end
                                            end)
                                            ...
                                            workspace.FallenPartsDestroyHeight = oldFP
                                            myRoot.CFrame = savedPos
                                            myRoot.Velocity = Vector3.zero; myRoot.RotVelocity = Vector3.zero
                                        end)
                                        self._flingBusy = false
                                    end)
                                    task.wait(0.1)
                                end
                            end
                ...
                if self._flingTask then
                    pcall(task.cancel, self._flingTask); self._flingTask = nil
                end
                _pc(function()
                    local c = _character(); if not c then return end
                    local r = c:FindFirstChild("HumanoidRootPart")
        ...
        function self:Stop()
            self:SetLoopTP(false); self:SetLoopBring(false); self:SetFling(false)
        end
        return self
    end

    -- ===== 飞行(完整保留原脚本猫飞行:自定义 GUI、超人姿势、漂浮按钮、按 F 切换) =====
    function G.Fly(opts)
        opts = opts or {}
        local self = setmetatable({ _opts = opts }, G)
        ...
        local maxSpeed = opts.maxSpeed or 1000
        local flySpeed = opts.speed or 100
        local isFlying = false
        local flyDir, joyDir = Vector3.zero, Vector3.zero
        local flyGyro, flyVel, flyParticles = nil, nil, nil
        local origMotorOffsets, savedPose = {}, false
        local gui = {}

        local function saveOrig()
            origMotorOffsets = {}
            for _, p in pairs(character:GetDescendants()) do
                if p:IsA("Motor6D") then origMotorOffsets[p] = { C0 = p.C0, C1 = p.C1 } end
            end
            savedPose = true
        end

        local function getJoints()
            local j = { isR15 = not character:FindFirstChild("Torso") }
            if j.isR15 then
                j.rootJoint = (character:FindFirstChild("LowerTorso") or character:FindFirstChild("HumanoidRootPart"))
                    and ((character.LowerTorso and character.LowerTorso:FindFirstChild("Root"))
                        or character.HumanoidRootPart:FindFirstChild("RootJoint")
                        or character.HumanoidRootPart:FindFirstChild("Root"))
                j.neck = character:FindFirstChild("Head")
                    and ((character.Head and character.Head:FindFirstChild("Neck"))
                        or (character:FindFirstChild("UpperTorso") and character.UpperTorso:FindFirstChild("Neck")))
                j.leftShoulder = character:FindFirstChild("UpperTorso") and character.UpperTorso:FindFirstChild("LeftShoulder")
                j.rightShoulder = character:FindFirstChild("UpperTorso") and character.UpperTorso:FindFirstChild("RightShoulder")
                j.leftElbow = character:FindFirstChild("LeftUpperArm") and character.LeftUpperArm:FindFirstChild("LeftElbow")
                j.rightElbow = character:FindFirstChild("RightUpperArm") and character.RightUpperArm:FindFirstChild("RightElbow")
                j.leftHip = character:FindFirstChild("LowerTorso") and character.LowerTorso:FindFirstChild("LeftHip")
                j.rightHip = character:FindFirstChild("LowerTorso") and character.LowerTorso:FindFirstChild("RightHip")
                j.leftKnee = character:FindFirstChild("LeftUpperLeg") and character.LeftUpperLeg:FindFirstChild("LeftKnee")
                j.rightKnee = character:FindFirstChild("RightUpperLeg") and character.RightUpperLeg:FindFirstChild("RightKnee")
                j.waist = character:FindFirstChild("UpperTorso") and character.UpperTorso:FindFirstChild("Waist")
            else
                j.rootJoint = character:FindFirstChild("HumanoidRootPart") and character.HumanoidRootPart:FindFirstChild("RootJoint")
                j.neck = character:FindFirstChild("Torso") and character.Torso:FindFirstChild("Neck")
                j.leftShoulder = character:FindFirstChild("Torso") and character.Torso:FindFirstChild("Left Shoulder")
                j.rightShoulder = character:FindFirstChild("Torso") and character.Torso:FindFirstChild("Right Shoulder")
                j.leftHip = character:FindFirstChild("Torso") and character.Torso:FindFirstChild("Left Hip")
                j.rightHip = character:FindFirstChild("Torso") and character.Torso:FindFirstChild("Right Hip")
            end
            return j
        end

        local function supermanPose()
            if not savedPose then return end
            local j = getJoints()
            local function set(name, rot)
                if j[name] and origMotorOffsets[j[name]] then
                    j[name].C0 = origMotorOffsets[j[name]].C0 * CFrame.Angles(math.rad(rot), 0, 0)
                end
            end
            if j.rootJoint and origMotorOffsets[j.rootJoint] then
                j.rootJoint.C0 = origMotorOffsets[j.rootJoint].C0 * CFrame.Angles(math.rad(-50), 0, 0)
            end
            if j.waist and origMotorOffsets[j.waist] then
                j.waist.C0 = origMotorOffsets[j.waist].C0 * CFrame.Angles(math.rad(-15), 0, 0)
            end
            set("neck", 55)
            if j.leftShoulder and origMotorOffsets[j.leftShoulder] then
                j.leftShoulder.C0 = origMotorOffsets[j.leftShoulder].C0 * CFrame.Angles(math.rad(-100), 0, math.rad(-15))
            end
            if j.rightShoulder and origMotorOffsets[j.rightShoulder] then
                j.rightShoulder.C0 = origMotorOffsets[j.rightShoulder].C0 * CFrame.Angles(math.rad(-100), 0, math.rad(15))
            end
            set("leftElbow", -15); set("rightElbow", -15)
            set("leftHip", 50); set("rightHip", 50)
            set("leftKnee", -25); set("rightKnee", -25)
        end

        local function restorePose()
            for m, off in pairs(origMotorOffsets) do
                if m and m.Parent then m.C0 = off.C0; m.C1 = off.C1 end
            end
        end

        local function mkTrails()
            if not rootPart then return end
            if rootPart:FindFirstChild("CatUI_FlyTrail") then rootPart.CatUI_FlyTrail:Destroy() end
            local folder = Instance.new("Folder"); folder.Name = "CatUI_FlyTrail"; folder.Parent = rootPart
            local al = Instance.new("Attachment"); al.Name = "L"; al.Position = Vector3.new(-0.7,-1.25,1.05); al.Parent = rootPart
            local ar = Instance.new("Attachment"); ar.Name = "R"; ar.Position = Vector3.new(0.7,-1.25,1.05); ar.Parent = rootPart
            local t1 = Instance.new("Trail")
            t1.Attachment0 = al; t1.Attachment1 = ar; t1.Lifetime = 0.85; t1.LightEmission = 0.18
            t1.FaceCamera = true
            t1.Color = ColorSequence.new({
            ...
            t1.Transparency = NumberSequence.new(0.45,0.62,1)
            t1.WidthScale = NumberSequence.new(0.95,0.7,0)
            t1.Parent = folder
            local t2 = Instance.new("Trail")
            t2.Attachment0 = al; t2.Attachment1 = ar; t2.Lifetime = 0.55; t2.LightEmission = 0.08
            t2.FaceCamera = true
            t2.Color = ColorSequence.new(Color3.fromRGB(170,125,125), Color3.fromRGB(120,80,80))
            t2.Transparency = NumberSequence.new(0.6,0.72,1)
            t2.WidthScale = NumberSequence.new(0.3,0.16,0)
            t2.Parent = folder
            return {folder, al, ar, t1, t2}
        end

        local function clearTrails()
            if flyParticles then
                for _, o in ipairs(flyParticles) do if o and o.Parent then o:Destroy() end end
                flyParticles = nil
            end
            if rootPart and rootPart:FindFirstChild("CatUI_FlyTrail") then rootPart.CatUI_FlyTrail:Destroy() end
        end

        local function mkGUI()
            local pg = player:WaitForChild("PlayerGui")
            local sg = Instance.new("ScreenGui")
            sg.Name = "CatUI_FlyGUI"; sg.ResetOnSpawn = false
            sg.ZIndexBehavior = Enum.ZIndexBehavior.Sibling; sg.Parent = pg
            gui.sg = sg

            local fb = Instance.new("ImageButton")
            fb.Name = "FloatBtn"; fb.Size = UDim2.new(0,40,0,40); fb.Position = UDim2.new(0,20,0,60)
            fb.BackgroundColor3 = Color3.fromRGB(20,30,50); fb.BackgroundTransparency = 0.1
            fb.Image = "rbxassetid://75293061485556"; fb.BorderSizePixel = 0
            fb.AutoButtonColor = false; fb.Visible = false; fb.Parent = sg
            gui.fb = fb
            Instance.new("UICorner", fb).CornerRadius = UDim.new(1,0)
            local fs = Instance.new("UIStroke"); fs.Color = Color3.fromRGB(100,220,255)
            fs.Transparency = 0.5; fs.Thickness = 2; fs.Parent = fb
            TweenService:Create(fs, TweenInfo.new(1.5, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {Transparency = 0.2}):Play()

            local pf = Instance.new("Frame")
            pf.Name = "Panel"; pf.Size = UDim2.new(0,160,0,195); pf.Position = UDim2.new(0.5,-80,0.5,-97)
            pf.BackgroundColor3 = Color3.fromRGB(20,30,50); pf.BackgroundTransparency = 0.15
            pf.BorderSizePixel = 0; pf.Visible = true; pf.Parent = sg
            gui.pf = pf
            Instance.new("UICorner", pf).CornerRadius = UDim.new(0,16)
            Instance.new("UIStroke", pf).Transparency = 0.7

            local minB = Instance.new("TextButton")
            minB.Size = UDim2.new(0,20,0,20); minB.Position = UDim2.new(0,8,0,6)
            minB.BackgroundColor3 = Color3.fromRGB(80,85,100); minB.BackgroundTransparency = 0.3
            minB.Text = "-"; minB.TextColor3 = Color3.new(1,1,1); minB.TextSize = 14
            minB.Font = Enum.Font.GothamBold; minB.BorderSizePixel = 0; minB.Parent = pf
            Instance.new("UICorner", minB).CornerRadius = UDim.new(0,4)

            local ttl = Instance.new("TextLabel")
            ttl.Size = UDim2.new(1,-56,0,26); ttl.Position = UDim2.new(0,28,0,6)
            ttl.BackgroundTransparency = 1; ttl.Text = "猫飞行"; ttl.TextColor3 = Color3.new(1,1,1)
            ttl.TextSize = 14; ttl.Font = Enum.Font.GothamBold; ttl.Parent = pf

            local xb = Instance.new("TextButton")
            xb.Size = UDim2.new(0,20,0,20); xb.Position = UDim2.new(1,-24,0,6)
            xb.BackgroundColor3 = Color3.fromRGB(255,70,70); xb.Text = "X"
            xb.TextColor3 = Color3.new(1,1,1); xb.TextSize = 12
            xb.Font = Enum.Font.GothamBold; xb.BorderSizePixel = 0; xb.Parent = pf
            Instance.new("UICorner", xb).CornerRadius = UDim.new(0,6)

            local cd = Instance.new("Frame")
            cd.Size = UDim2.new(1,0,1,0); cd.BackgroundColor3 = Color3.fromRGB(20,30,50)
            cd.BackgroundTransparency = 0.05; cd.BorderSizePixel = 0
            cd.ZIndex = 10; cd.Visible = false; cd.Parent = pf
            Instance.new("UICorner", cd).CornerRadius = UDim.new(0,12)

            local ct = Instance.new("TextLabel")
            ct.Size = UDim2.new(1,0,0,30); ct.Position = UDim2.new(0,0,0,28)
            ct.BackgroundTransparency = 1; ct.Text = "关闭飞行"; ct.TextColor3 = Color3.new(1,1,1)
            ct.TextSize = 15; ct.Font = Enum.Font.GothamBold; ct.ZIndex = 11; ct.Parent = cd

            local cx = Instance.new("TextLabel")
            cx.Size = UDim2.new(1,-24,0,36); cx.Position = UDim2.new(0,12,0,62)
            cx.BackgroundTransparency = 1; cx.Text = "是否要关闭猫飞行?您将无法再次打开它"
            cx.TextColor3 = Color3.fromRGB(180,185,195); cx.TextSize = 12
            cx.Font = Enum.Font.Gotham; cx.TextWrapped = true; cx.ZIndex = 11; cx.Parent = cd

            local bc = Instance.new("Frame")
            bc.Size = UDim2.new(1,-24,0,34); bc.Position = UDim2.new(0,12,1,-50)
            bc.BackgroundTransparency = 1; bc.ZIndex = 11; bc.Parent = cd
            local bl = Instance.new("UIListLayout"); bl.FillDirection = Enum.FillDirection.Horizontal
            bl.Padding = UDim.new(0,10); bl.HorizontalAlignment = Enum.HorizontalAlignment.Center; bl.Parent = bc

            local cb = Instance.new("TextButton")
            cb.Size = UDim2.new(0.5,-5,1,0); cb.BackgroundColor3 = Color3.fromRGB(55,58,65)
            cb.Text = "取消"; cb.TextColor3 = Color3.new(1,1,1); cb.TextSize = 13
            cb.Font = Enum.Font.GothamBold; cb.BorderSizePixel = 0; cb.ZIndex = 11; cb.Parent = bc
            Instance.new("UICorner", cb).CornerRadius = UDim.new(0,8)

            local cfb = Instance.new("TextButton")
            cfb.Size = UDim2.new(0.5,-5,1,0); cfb.BackgroundColor3 = Color3.fromRGB(200,60,60)
            cfb.Text = "关闭飞行"; cfb.TextColor3 = Color3.new(1,1,1); cfb.TextSize = 13
            cfb.Font = Enum.Font.GothamBold; cfb.BorderSizePixel = 0; cfb.ZIndex = 11; cfb.Parent = bc
            Instance.new("UICorner", cfb).CornerRadius = UDim.new(0,8)

            local cf = Instance.new("Frame")
            cf.Size = UDim2.new(1,-16,0,142); cf.Position = UDim2.new(0,8,0,38)
            cf.BackgroundTransparency = 1; cf.Parent = pf
            local ll = Instance.new("UIListLayout"); ll.Padding = UDim.new(0,8)
            ll.SortOrder = Enum.SortOrder.LayoutOrder; ll.Parent = cf

            local tr = Instance.new("Frame")
            tr.Size = UDim2.new(1,0,0,36); tr.BackgroundColor3 = Color3.fromRGB(40,45,60)
            tr.BackgroundTransparency = 0.3; tr.BorderSizePixel = 0
            tr.LayoutOrder = 1; tr.Parent = cf
            Instance.new("UICorner", tr).CornerRadius = UDim.new(0,10)

            local al = Instance.new("TextLabel")
            al.Size = UDim2.new(0,20,1,0); al.Position = UDim2.new(0,8,0,0)
            al.BackgroundTransparency = 1; al.Text = "≫"; al.TextColor3 = Color3.fromRGB(100,220,255)
            al.TextSize = 14; al.Font = Enum.Font.GothamBold; al.Parent = tr

            local tt = Instance.new("TextLabel")
            tt.Size = UDim2.new(0,90,1,0); tt.Position = UDim2.new(0,28,0,0)
            tt.BackgroundTransparency = 1; tt.Text = "开启飞行"
            tt.TextColor3 = Color3.new(1,1,1); tt.TextSize = 13
            tt.Font = Enum.Font.GothamBold; tt.TextXAlignment = Enum.TextXAlignment.Left; tt.Parent = tr
            gui.tt = tt

            local tb = Instance.new("TextButton")
            tb.Size = UDim2.new(0,40,0,22); tb.Position = UDim2.new(1,-48,0.5,-11)
            tb.BackgroundColor3 = Color3.new(1,1,1); tb.BackgroundTransparency = 0.85
            tb.Text = ""; tb.BorderSizePixel = 0; tb.AutoButtonColor = false; tb.Parent = tr
            gui.tb = tb
            Instance.new("UICorner", tb).CornerRadius = UDim.new(1,0)
            Instance.new("UIStroke", tb).Transparency = 0.8
            local td = Instance.new("Frame")
            td.Size = UDim2.new(0,16,0,16); td.Position = UDim2.new(0,3,0.5,-8)
            td.BackgroundColor3 = Color3.new(1,1,1); td.BorderSizePixel = 0; td.Parent = tb
            gui.td = td
            Instance.new("UICorner", td).CornerRadius = UDim.new(1,0)
            tb.MouseButton1Click:Connect(function() self:Toggle() end)

            local sr = Instance.new("Frame")
            sr.Size = UDim2.new(1,0,0,48); sr.BackgroundColor3 = Color3.fromRGB(40,45,60)
            sr.BackgroundTransparency = 0.3; sr.BorderSizePixel = 0
            sr.LayoutOrder = 2; sr.Parent = cf
            Instance.new("UICorner", sr).CornerRadius = UDim.new(0,10)
            local sl = Instance.new("TextLabel")
            sl.Size = UDim2.new(0,50,0,20); sl.Position = UDim2.new(0,10,0,5)
            sl.BackgroundTransparency = 1; sl.Text = "速度"; sl.TextColor3 = Color3.new(1,1,1)
            sl.TextSize = 13; sl.Font = Enum.Font.GothamBold; sl.TextXAlignment = Enum.TextXAlignment.Left; sl.Parent = sr
            local sv = Instance.new("TextLabel")
            sv.Size = UDim2.new(0,50,0,20); sv.Position = UDim2.new(1,-58,0,5)
            sv.BackgroundTransparency = 1; sv.Text = "0%"; sv.TextColor3 = Color3.fromRGB(100,220,255)
            sv.TextSize = 13; sv.Font = Enum.Font.GothamBold; sv.TextXAlignment = Enum.TextXAlignment.Right; sv.Parent = sr
            gui.sv = sv
            local st = Instance.new("Frame")
            st.Size = UDim2.new(1,-20,0,6); st.Position = UDim2.new(0,10,0,32)
            st.BackgroundColor3 = Color3.fromRGB(60,65,80); st.BorderSizePixel = 0; st.Parent = sr
            Instance.new("UICorner", st).CornerRadius = UDim.new(0,3)
            local sf = Instance.new("Frame")
            sf.Size = UDim2.new(0,0,1,0); sf.BackgroundColor3 = Color3.fromRGB(100,220,255)
            sf.BorderSizePixel = 0; sf.Parent = st
            gui.sf = sf
            Instance.new("UICorner", sf).CornerRadius = UDim.new(0,3)
            local sbt = Instance.new("TextButton")
            sbt.Size = UDim2.new(0,14,0,14); sbt.Position = UDim2.new(0,-7,0.5,-7)
            sbt.BackgroundColor3 = Color3.new(1,1,1); sbt.Text = ""; sbt.BorderSizePixel = 0; sbt.Parent = st
            gui.sbt = sbt
            Instance.new("UICorner", sbt).CornerRadius = UDim.new(1,0)
            local drag = false
            local function updateSpd(input)
                local pos = st.AbsolutePosition.X; local sz = st.AbsoluteSize.X
                local rel = math.clamp((input.Position.X - pos) / sz, 0, 1)
                sbt.Position = UDim2.new(rel,-7,0.5,-7); sf.Size = UDim2.new(rel,0,1,0)
                flySpeed = math.floor(minSpeed + (maxSpeed - minSpeed) * rel)
                sv.Text = tostring(math.floor(rel*100)).."%"
            end
            sbt.InputBegan:Connect(function(i)
            ...
            st.InputBegan:Connect(function(i)
                if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then drag = true; updateSpd(i) end
            end)
            ...
            hr.Size = UDim2.new(1,0,0,36); hr.BackgroundColor3 = Color3.fromRGB(40,45,60)
            hr.BackgroundTransparency = 0.3; hr.BorderSizePixel = 0
            hr.LayoutOrder = 3; hr.Parent = cf
            Instance.new("UICorner", hr).CornerRadius = UDim.new(0,10)
            local hl = Instance.new("TextLabel")
            hl.Size = UDim2.new(0,50,1,0); hl.Position = UDim2.new(0,10,0,0)
            hl.BackgroundTransparency = 1; hl.Text = "高度"; hl.TextColor3 = Color3.new(1,1,1)
            hl.TextSize = 13; hl.Font = Enum.Font.GothamBold; hl.TextXAlignment = Enum.TextXAlignment.Left; hl.Parent = hr
            local hv = Instance.new("TextLabel")
            hv.Size = UDim2.new(0,60,1,0); hv.Position = UDim2.new(1,-68,0,0)
            hv.BackgroundTransparency = 1; hv.Text = "0"; hv.TextColor3 = Color3.fromRGB(100,220,255)
            hv.TextSize = 13; hv.Font = Enum.Font.GothamBold; hv.TextXAlignment = Enum.TextXAlignment.Right; hv.Parent = hr
            gui.hv = hv

            local pDrag, pStart, pFrame = false, nil, nil
            pf.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    local a = pf.AbsolutePosition; local s = pf.AbsoluteSize
                    local m = Vector2.new(input.Position.X, input.Position.Y)
                    local rx = m.X - a.X; local ry = m.Y - a.Y
                    if rx > (s.X - 30) and ry < 30 then return end
                    if rx > (s.X - 60) and ry > 40 and ry < 80 then return end
                    if ry > 85 and ry < 140 then return end
                    pDrag = true; pStart = m; pFrame = pf.Position
                end
            end)
            UserInputService.InputEnded:Connect(function(i)
                if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then pDrag = false end
            end)
            UserInputService.InputChanged:Connect(function(i)
                if not pDrag then return end
                if i.UserInputType ~= Enum.UserInputType.MouseMovement and i.UserInputType ~= Enum.UserInputType.Touch then return end
                local d = Vector2.new(i.Position.X, i.Position.Y) - pStart
                pf.Position = UDim2.new(pFrame.X.Scale, pFrame.X.Offset + d.X, pFrame.Y.Scale, pFrame.Y.Offset + d.Y)
            end)

            local panelOpen = false
            local function togglePanel()
                panelOpen = not panelOpen
                pf.Visible = panelOpen
                if panelOpen then
                    fb.Visible = false
                    pf.Position = UDim2.new(0.5,-80,0.5,-97)
                    pf.Size = UDim2.new(0,0,0,0)
                    TweenService:Create(pf, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Size = UDim2.new(0,160,0,195) }):Play()
                end
            end
            fb.MouseButton1Click:Connect(function() togglePanel() end)
            minB.MouseButton1Click:Connect(function()
                panelOpen = false; pf.Visible = false; cd.Visible = false
                fb.Position = UDim2.new(0,20,0,60); fb.Visible = true
            end)
            xb.MouseButton1Click:Connect(function() cd.Visible = true end)
            cb.MouseButton1Click:Connect(function() cd.Visible = false end)
            cfb.MouseButton1Click:Connect(function()
                if isFlying then self:Set(false) end
                if sg and sg.Parent then sg:Destroy() end
                gui = {}
            end)

            local fDrag, fStart, fFrame = false, nil, nil
            fb.InputBegan:Connect(function(i)
                if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
                    fDrag = true; fStart = Vector2.new(i.Position.X, i.Position.Y); fFrame = fb.Position
                end
            end)
            UserInputService.InputEnded:Connect(function(i)
            ...
            UserInputService.InputChanged:Connect(function(i)
                ...
                if i.UserInputType ~= Enum.UserInputType.MouseMovement and i.UserInputType ~= Enum.UserInputType.Touch then return end
                local d = Vector2.new(i.Position.X, i.Position.Y) - fStart
                fb.Position = UDim2.new(fFrame.X.Scale, fFrame.X.Offset + d.X, fFrame.Y.Scale, fFrame.Y.Offset + d.Y)
            end)
            UserInputService.InputBegan:Connect(function(i, gp)
                if gp then return end
                if i.KeyCode == Enum.KeyCode.F then togglePanel() end
            end)
        end

        local function updateGUI()
            if not gui.tb then return end
            local ti = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            if isFlying then
                gui.tt.Text = "关闭飞行"; gui.tt.TextColor3 = Color3.fromRGB(100,220,255)
                TweenService:Create(gui.td, ti, { Position = UDim2.new(1,-19,0.5,-8) }):Play()
                TweenService:Create(gui.tb, ti, { BackgroundColor3 = Color3.fromRGB(100,220,255) }):Play()
            else
                gui.tt.Text = "开启飞行"; gui.tt.TextColor3 = Color3.new(1,1,1)
                TweenService:Create(gui.td, ti, { Position = UDim2.new(0,3,0.5,-8) }):Play()
                TweenService:Create(gui.tb, ti, { BackgroundColor3 = Color3.new(1,1,1) }):Play()
            end
        end
        ...
        local function startFly()
            ...
            isFlying = true
            flyGyro = Instance.new("BodyGyro"); flyGyro.Parent = rootPart
            flyGyro.MaxTorque = Vector3.new(9e9,9e9,9e9)
            flyGyro.D = 500; flyGyro.P = 50000
            flyGyro.CFrame = rootPart.CFrame
            flyVel = Instance.new("BodyVelocity"); flyVel.Parent = rootPart
            flyVel.MaxForce = Vector3.new(9e9,9e9,9e9); flyVel.Velocity = Vector3.zero
            humanoid.PlatformStand = true
            if not savedPose then saveOrig() end
            supermanPose()
            flyParticles = mkTrails()
            updateGUI()
        end
        ...
        local function stopFly()
            ...
            restorePose(); clearTrails()
            flyDir = Vector3.zero; joyDir = Vector3.zero
            updateGUI()
        end
        ...
        humanoid:GetPropertyChangedSignal("MoveDirection"):Connect(function()
            ...
            joyDir = Vector3.new(md.X, 0, md.Z)
        end)
        UserInputService.InputBegan:Connect(function(i, gp)
            if gp or not isFlying then return end
            local k = i.KeyCode
            if k == Enum.KeyCode.W then flyDir = flyDir + Vector3.new(0,0,-1) end
            if k == Enum.KeyCode.S then flyDir = flyDir + Vector3.new(0,0, 1) end
            if k == Enum.KeyCode.A then flyDir = flyDir + Vector3.new(-1,0,0) end
            if k == Enum.KeyCode.D then flyDir = flyDir + Vector3.new( 1,0,0) end
        end)
        ...
        RunService.RenderStepped:Connect(function()
            if gui.hv and rootPart then gui.hv.Text = tostring(math.floor(rootPart.Position.Y)) end
            if isFlying and savedPose then supermanPose() end
            if not isFlying or not flyVel or not flyGyro then return end
            local cam = workspace.CurrentCamera
            local cf = cam.CFrame; local look = cf.LookVector; local right = cf.RightVector
            local ix = flyDir.X + joyDir:Dot(right)
            local iz = -flyDir.Z + joyDir:Dot(look)
            local fd = right * ix + look * iz
            if fd.Magnitude > 0 then fd = fd.Unit * flySpeed end
            flyVel.Velocity = fd
            local tLook = Vector3.new(look.X,0,look.Z)
            if tLook.Magnitude > 0 then
                tLook = tLook.Unit
                flyGyro.CFrame = CFrame.new(rootPart.Position, rootPart.Position + tLook)
            end
        end)
        player.CharacterAdded:Connect(function(nc)
            ...
            savedPose = false; origMotorOffsets = {}
            humanoid.Died:Connect(function() stopFly() end)
        end)
        humanoid.Died:Connect(function() stopFly() end)
        mkGUI(); updateGUI()

        function self:Set(v) if v then startFly() else stopFly() end end
        function self:Toggle() if isFlying then self:Set(false) else self:Set(true) end end
        function self:Stop() stopFly() end
        return self
    end
